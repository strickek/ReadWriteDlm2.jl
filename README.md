...
Base `writedlm()` writes `3000.0` always short as `3e3`. Function `writedlm2()` writes long 
like print() by default. Set `write_short=true` to arrive at the same result as with Base `writedlm()`.
