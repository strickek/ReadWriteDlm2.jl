...
The functions `readdlm2()` and `writedlm2()` of module `ReadWriteDlm2` are similar to `readdlm()` and `writedlm()` of Julia Base.  Differences in usage are: `';'` as default delimiter, `','` as default decimal mark. The basic idea of this functions is to support the "decimal comma countries". 

Because of additional capabilities for Date, DateTime, Time, Complex and Rational types, `ReadWriteDlm2` is also useful for "decimal dot" users. The can use readdlm2/writdlm2 with the respective delimiter argument (`' '`, `'\t'` or `','`) and `decimal='.'`. More convinient are `readcsv2` and `writecsv2` because they have already the right defaults (`','` as delimiter, `Any` as Type and  `','` as decimal mark).
